...
flowchart TD
    ...
    E -->|CLI| C[CLI Controller]
    E -->|Gateway| G[Gateway Runner]
    E -->|Cron| CR[Cron Scheduler]
    E -->|Delegate| D[Delegate Job]

    C --> P[Prompt Builder]
    G --> P
    CR --> CA[Cron Agent Session]
    D --> DA[Delegate Agent Session]

    P --> L[Conversation AgentLoop]
    CA --> L
    DA --> L
...
```text
用户输入
  → CLI Controller 或 Gateway Runner
  → 组装 SOUL、Memory、Skills 和项目上下文
  → ConversationAgentLoop
  → 调用模型
  → 解析并执行工具
  → 持久化消息与工具结果
  → 返回最终回复
  → 按条件提交后台 Memory/Skill Review
```

Cron 和 Delegate 会创建独立 AgentLoop，而不是直接在主会话中执行一段特殊逻辑。
