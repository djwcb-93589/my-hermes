...
class PageExtractor:
            ...
            """(input) => {
                ...
                const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
                    acceptNode(node) {
                        ...
                        if (!parent || ignored.has(parent.tagName) || !node.nodeValue.trim()) {
        ...
        raw_links = self._ensure_list(
            self._evaluate(
                """(limit) => {
                    ...
                    return links.slice(0, limit + 1).map((element, index) => {
                    ...
                    return Array.from(document.querySelectorAll('table')).slice(0, input.limit + 1)
                        .map((table, index) => {
                            ...
                            const rows = allRows.filter((row) => row !== headerRow)
                                .slice(0, input.rowLimit + 1)
                                .map((row) => Array.from(row.cells).map(text));
                            return {
                                table_index: index,
                                caption: text(table.caption),
                                headers,
                                rows,
                                truncated: allRows.filter((row) => row !== headerRow).length > input.rowLimit
                            };
                        });
                }""",
                {"limit": limit, "rowLimit": min(limit, _MAX_TABLE_ROWS)},
            ),
            name="表格提取",
        )
    ...
    def extract_forms(self, max_items: int) -> dict[str, Any]:
            ...
            self._evaluate(
                """(limit) => {
                    ...
                    const labelOf = (element) => {
                        ...
                        if (labels.length) return labels.join(' ').replace(/\s+/g, ' ').trim();
                        const parentLabel = element.closest('label');
                        return (element.getAttribute('aria-label') || parentLabel?.innerText || '').replace(/\s+/g, ' ').trim();
                    };
                    return Array.from(document.forms).slice(0, limit + 1).map((form, index) => ({
                        form_index: index,
                        action: form.getAttribute('action') ? new URL(form.getAttribute('action'), document.baseURI).href : document.URL,
                        ...
                        fields: Array.from(form.elements).filter((element) =>
                            ...
                            return {
                                name: element.getAttribute('name') || '', type,
                                label: labelOf(element), required: Boolean(element.required),
                                value: value.slice(0, 2000), _index: controlIndex
                            };
                        })
                    }));
                }""",
        ...
        return {"forms": forms, "count": len(forms), "truncated": len(raw_forms) > limit}
        ...
        result = self._evaluate(
            """() => {
                ...
                for (const element of document.querySelectorAll('meta[name], meta[property]')) {
                    ...
                    if (key.toLowerCase().startsWith('og:')) openGraph[key] = value;
                    else meta[key] = value;
                }
                const jsonLd = [];
    ...
    def _extract_kind(self, kind: str, max_items: int) -> tuple[list[dict[str, Any]], bool]:
        if kind == "links":
            result = self.extract_links(max_items)
            return result["links"], result["truncated"]
        if kind == "tables":
            result = self.extract_tables(max_items)
            return result["tables"], result["truncated"]
        if kind == "forms":
            result = self.extract_forms(max_items)
        ...
        candidate = self._evaluate(
            """() => {
                ...
                for (const element of document.querySelectorAll('a[href]')) {
                    if (nextNames.has(named(element))) { const found = link(element, 2); if (found) return found; }
                }
                const buttons = Array.from(document.querySelectorAll('button, [role="button"]'));
                for (let index = 0; index < buttons.length; index += 1) {
                    const element = buttons[index];
                    if (!element.matches(':disabled') && element.getAttribute('aria-disabled') !== 'true' && nextNames.has(named(element))) {
                        return {kind: 'button', index, priority: 2};
                    }
                }
                const regions = document.querySelectorAll('[role="navigation"], nav, .pagination, [aria-label*="pagination" i]');
                for (const region of regions) {
                    for (const element of region.querySelectorAll('a[href]')) {
                        if (nextNames.has(named(element))) { const found = link(element, 3); if (found) return found; }
                    }
                    for (const element of region.querySelectorAll('button, [role="button"]')) {
                        const index = buttons.indexOf(element);
                        if (index >= 0 && !element.matches(':disabled') && element.getAttribute('aria-disabled') !== 'true' && nextNames.has(named(element))) {
        ...
        *,
        ...
        while True:
            ...
            if remaining_items <= 0:
                ...
                truncated = True
                break
            page_items, page_truncated = self._extract_kind(extract_kind, remaining_items)
